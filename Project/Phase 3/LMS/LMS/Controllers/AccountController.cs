...
/// 
/// This file is a method that works with login and create new users
/// 
/// </summary>
namespace LMS.Controllers
{
    ...
    public class AccountController : CommonController
    {
        ...
        public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
        {
            ...
            switch (role)
            {
                ...
                case "Professor":
                    ...
                    prof.Dob = DOB;
                    prof.Id = UID;
                    prof.SubjectAbbreviation = SubjectAbbrev;
                    db.Add(prof);
                    break;
                case "Student":
                    ...
                    std.Dob = DOB;
                    std.Id = UID;
                    std.SubjectAbbreviation = SubjectAbbrev;
                    db.Add(std);
                    break;
            }
            db.SaveChanges();
        ...
        /// <summary>
        /// A helper method to return the next new uid
        /// </summary>
        /// <returns>A unique uID that is not be used by anyone else</returns>
        private string generateUID()
        {
            ...
            if (queryA == null && queryP == null && queryS == null)
            {
                uID = "u0000000";
            }
            else
            {
                if ((queryA != null && queryP != null) && String.Compare(queryA, queryP) > 0)
                {
                    maxuID = queryA;
                }
                else if (queryP != null)
                ...
                if (maxuID == null && queryS != null)
                {
                    maxuID = queryS;
                }
                else if (maxuID != null && queryS == null)
                {
                }
                else if (String.Compare(queryS, maxuID) > 0)
                {
                    maxuID = queryS;
                }

                int id = Int32.Parse(maxuID.Substring(1)) + 1;
                string idStr = id.ToString();
                uID = "u" + idStr.PadLeft(8 - idStr.Length, '0');
            }
