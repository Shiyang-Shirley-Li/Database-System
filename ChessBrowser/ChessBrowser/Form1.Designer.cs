...
/// 
/// This file is to build relaltions between UI and database
/// </summary>
namespace ChessBrowser
{
    partial class Form1
    {
        ...
        private void UploadGamesToDatabase(string PGNfilename)
        {
            ...
            List<ChessGame> chGames = pgn.ReadPGNFile(PGNfilename);
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    foreach (ChessGame game in chGames)
                    {
                        //Players: Name, Elo, pID(auto)
                        MySqlCommand commandPlayersB = conn.CreateCommand();
                        commandPlayersB.CommandText = "INSERT IGNORE INTO Players (Name, Elo) VALUES (@name, @elo) " +
                            "ON DUPLICATE KEY UPDATE Elo = IF(Elo < @elo, @elo, Elo);";
                        commandPlayersB.Parameters.AddWithValue("@name", game.BlackPlayerName);
                        commandPlayersB.Parameters.AddWithValue("@elo", game.BlackElo);
                        commandPlayersB.ExecuteNonQuery();
                        commandPlayersB.Prepare();//tell the server compile and save it

                        MySqlCommand commandPlayersW = conn.CreateCommand();
                        commandPlayersW.CommandText = "INSERT IGNORE INTO Players (Name, Elo) VALUES (@name, @elo) " +
                            "ON DUPLICATE KEY UPDATE Elo = IF(Elo < @elo, @elo, Elo);";
                        commandPlayersW.Parameters.AddWithValue("@name", game.WhitePlayerName);
                        commandPlayersW.Parameters.AddWithValue("@elo", game.WhiteElo);
                        commandPlayersW.ExecuteNonQuery();

                        commandPlayersW.Prepare();

                        //Events: Name, Site, Date, eID(auto)
                        MySqlCommand commandEvents = conn.CreateCommand();
                        commandEvents.CommandText = "INSERT IGNORE INTO Events (Name, Site, Date) VALUES (@name, @site, @date) ";
                        commandEvents.Parameters.AddWithValue("@name", game.EventName);
                        commandEvents.Parameters.AddWithValue("@site", game.EventSite);
                        commandEvents.Parameters.AddWithValue("@date", game.EventDate);
                        commandEvents.ExecuteNonQuery();

                        commandEvents.Prepare();

                        //Games: Round, Result, Moves, Blackplayer(from Players), Whiteplayer(from Players), eID(from events)
                        MySqlCommand commandGames = conn.CreateCommand();
                        commandGames.CommandText = "INSERT IGNORE INTO Games (Round, Result, Moves, Blackplayer, Whiteplayer, eID) VALUES " +
                            "(@round, @result, @moves, " +
                        ...
                        commandGames.Parameters.AddWithValue("@round", game.Round);
                        commandGames.Parameters.AddWithValue("@result", game.Result);
                        commandGames.Parameters.AddWithValue("@moves", game.Moves);

                        commandGames.Parameters.AddWithValue("@blackplayer", game.BlackPlayerName);
                        commandGames.Parameters.AddWithValue("@whiteplayer", game.WhitePlayerName);
                        commandGames.Parameters.AddWithValue("@name", game.EventName);
                        commandGames.Parameters.AddWithValue("@site", game.EventSite);
                        commandGames.Parameters.AddWithValue("@date", game.EventDate);
          ...
          string winner, bool useDate, DateTime start, DateTime end, bool showMoves)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    //Basic SQL Query
                    command.CommandText = "SELECT DISTINCT Events.Name as eName, Site, Date, White.Name AS wName, Black.Name AS bName, Result, Moves, " +
                        "White.Elo AS wElo, Black.Elo AS bElo " +
                        "FROM Events JOIN Players Black JOIN Players White NATURAL JOIN Games WHERE Black.pID = BlackPlayer AND White.pID = WhitePlayer";

                    //Filters
                    if (white != "")
                    {
                        command.CommandText += " AND White.Name = @wplayerName";
                        command.Parameters.AddWithValue("@wplayerName", white);
                    }

                    if (black != "")
                    {
                        command.CommandText += " AND Black.Name = @bplayerName";
                        command.Parameters.AddWithValue("@bplayername", black);
                    }

                    if (opening != "")
                    {
                        command.CommandText += " AND Moves LIKE @start";
                        command.Parameters.AddWithValue("@start", opening + "%");
                    }

                    if (winner != "")
                    {
                        command.CommandText += " AND Result = @result";
                        command.Parameters.AddWithValue("@result", winner[0]);
                    }


                    if (useDate is true)
                    {
                        command.CommandText += " AND Date >= @start AND DATE <= @end";
                    ...
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            whiteplayer = (string)reader["wName"];
                            whiteElo = (UInt32)reader["wElo"];
                            blackplayer = (string)reader["bName"];
                            blackElo = (UInt32)reader["bElo"];

                            parsedResult += "Event: " + reader["eName"] + "\r\n";
                            parsedResult += "Site: " + reader["Site"] + "\r\n";
                            parsedResult += "Date: " + reader["Date"] + "\r\n";
                            parsedResult += "White: " + whiteplayer + " (" + whiteElo + ")" + "\r\n";
                            parsedResult += "Black: " + blackplayer + " (" + blackElo + ")" + "\r\n";
                            parsedResult += "Result: " + reader["Result"] + "\r\n";

                            if (showMoves is true)
                            {
